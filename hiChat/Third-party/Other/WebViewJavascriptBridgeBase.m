...
- (void)_log:(NSString *)action json:(id)json {
    ...
    NSString *prettyString = json;
    if (dic) {
        NSData *data = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:nil];
        prettyString = [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] stringByReplacingOccurrencesOfString:@" : " withString:@": "];
    }
    
    NSLog(@"WVJB %@: %@", action, prettyString);
}
